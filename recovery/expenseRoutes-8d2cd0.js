...
class ExpenseRoutes extends Crud {
  ...
  _delete(id, callback) {
    ...
    this.deleteCostFromBudget(expense.expenseTypeId, expense.userId, expense.cost, callback);
  }
  ...
  deleteCostFromBudget(expenseTypeId, userId, cost, callback) {
    ...
    if (!employee) {
      callback({
        msg: 'employee not found, deleted expense without removing any expense from employee',
        code: 404
      });
      console.log(employee);
